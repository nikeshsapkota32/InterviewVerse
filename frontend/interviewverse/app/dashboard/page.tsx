  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-5 text-sm font-medium text-primary-foreground transition-all hover:bg-primary/90 hover:shadow-[0_0_24px_rgba(163,230,53,0.35)]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Stat icon={Target} label="Sessions" value="42" delta="+8 this week" trend="up" />
        <Stat icon={TrendingUp} label="Avg score" value="78" suffix="/100" delta="+4.2" trend="up" />
        <Stat icon={Flame} label="Streak" value="6" suffix="days" delta="Personal best" trend="neutral" />
        <Stat icon={Clock} label="Practice" value="18.4" suffix="hrs" delta="Last 30 days" trend="neutral" />
      </div>
    ...
    <div className="rounded-2xl border border-border bg-card/40 lg:col-span-2">
      <div className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Your last 4 interviews</p>
        </div>
      ...
      <ul className="divide-y divide-border">
        {sessions.map((s) => {
          const Icon =
            s.type === "Technical" ? Terminal : s.type === "Behavioral" ? Mic : FileText;
          return (
            <li
              key={s.id}
...
function PerformanceChart() {
  const data = [62, 68, 65, 71, 74, 70, 73, 78, 76, 81, 79, 84, 82, 87];
  const w = 800;
  const h = 200;
  const min = 50;
  const max = 100;
